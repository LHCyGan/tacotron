...
For a basic Chinese model please check <https://github.com/boltomli/tacotron/releases/>. The model supports Pinyin with separating spaces. For eval and demo_server purpose, `g2pM` is introduced to input Chinese characters, i.e. 可以直接输入中文啦. See `eval.py` for samples. To try on GPU machine, set `CUDA_VISIBLE_DEVICES=""` for the next command to force CPU.
